...
impl TrackerView {
    pub fn view(&self) -> Element<Message> {
        match &self.state {
            ...
            State::Ripping {
                ...
                progress,
                total_errors,
            } => self.view_ripping(message, *progress, *total_errors),
            State::Finished { state, time } => self.view_finished(state, time),
        }
    }
    ...
    fn view_finished(&self, complete_state: &CompleteState, time: &Time) -> Element<Message> {
            ...
            CompleteState::TooMuchErrors { log, total } => {
                container(
                    column![
                        ...
                        button(text(log.display()).horizontal_alignment(Horizontal::Center))
                            .padding(0), // .on_press(Message::Open(log.to_owned()))
